...
def analyze_pose(
    ...
    try:
        ...
        if keypoints is None:
            logger.warning("MediaPipe pose detection returned None for image: %s", image_path)
            return {
                "score": 0,
                "corrections": [
        ...
        logger.error(f"Error in analyze_pose: {str(e)}", exc_info=True)
        return {
            "score": 0,
            "corrections": [f"Server error: {str(e)}"],
